...
export default function ReviewForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!form.message.trim()) {
      setError(t.feedback.writeAbout);
      setStatus("error");
      return;
    }
    setStatus("sending");
  ...
  if (status === "sent") {
    return (
      <div className="bg-white rounded-2xl p-8 border border-slate-200 text-center">
        ...
        <h3 className="text-2xl font-bold text-slate-900 mb-2">{t.feedback.sentTitle}</h3>
        <p className="text-slate-600">{t.feedback.sentBody}</p>
      </div>
    );
  }
    ...
    >
        ...
        <div className="flex gap-1.5" onMouseLeave={() => setHover(0)}>
          {[1, 2, 3, 4, 5].map((star) => (
            <button
              ...
              aria-label={t.feedback.starsLabel(star)}
            >
              <svg
      ...
      <div>
        <label htmlFor="rev-name" className="block text-sm font-semibold text-slate-700 mb-1.5">
          {t.feedback.name} <span className="text-slate-400 font-normal">{t.feedback.nameOptional}</span>
        </label>
        <input
          id="rev-name"
          type="text"
      ...
      <div>
        <label htmlFor="rev-message" className="block text-sm font-semibold text-slate-700 mb-1.5">
          {t.feedback.thoughts} <span className="text-red-500">*</span>
        </label>
        <textarea
          id="rev-message"
          required
      ...
      <div className="flex items-center justify-between gap-3 pt-1">
        <p className="text-xs text-slate-500">{t.feedback.notePublic}</p>
        <button
          type="submit"
